"""
File:          compressing_posting.py
Author:        Rishabh Dalal
Date:          27/02/2018
Description:   Enoding and decoding postings of a retrieval system using
               Variable byte encoding.

Modified:      2/03/2018 by Rishabh Dalal
Change:        Debugged the VBDecode function
...
def VBEncode(gap):
  ...
  while gap >= 128:
    byte.insert(0, int(gap%128))
    gap = gap/128
  byte.insert(0, int(gap))
  byte[len(byte)-1] += 128
  ...
  for i in byte:
    ...
    decimalRepr = decimal2binary(i)
    while len(decimalRepr) < 8:   ##Padding a binary number with 0s to make 8 digits
      decimalRepr = "0" + decimalRepr
    result += decimalRepr
